...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber uav1_local_pose_sub = nh.subscribe<geometry_msgs::PoseStamped>
            ("uav1/mavros/local_position/pose", 10, local_pose_cb);
    ...
    uav1_set_pose.pose.position.x = 0;
    uav1_set_pose.pose.position.y = 0;
    uav1_set_pose.pose.position.z = 4;
    ...
    while(ros::ok()){
        ...
        {
            uav1_set_pose.pose.position.x = 3 * sin(0.5 * run_time);
            uav1_set_pose.pose.position.y = 3 * cos(0.5 * run_time);
            uav1_set_pose.pose.position.z = 4;
        }
        uav1_local_pos_pub.publish(uav1_set_pose);
