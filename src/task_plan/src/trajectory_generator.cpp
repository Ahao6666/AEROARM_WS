...
trajectory_gen::trajectory_gen(){
  ...
  // 【服务】轨迹生成计算
  // 本话题的输入来自于CLIK，计算贝塞尔曲线的参数，储存在_bezier_中。
...
c_float lowerVelocity,c_float upperAccelera,c_float lowerAccelera,c_float x_0,c_float x_n,c_float v_0,c_float v_n){
  ...
  csc* CC = csc_matrix(CC_m, CC_n, CC_nzmax, CCvar, CCii, CCp);
    ...
    if (data) {
      data->n = n_x;
      data->m = CC->m;
      data->P = csc_matrix(data->n, data->n, P_mnz, P_var, P_raw, P_con); // P只需要记录上三角矩阵即可,认为是对称矩阵
      data->q = Pq;
      data->A = CC; // 需要全部记录
      data->l = l_a;
      data->u = u_a;
...
bool trajectory_gen::traj_solver_call(task_plan::traj_solver_msgRequest& request,task_plan::traj_solver_msgResponse& response){
...
bool trajectory_gen::traj_out_call(task_plan::traj_out_msgRequest& request,task_plan::traj_out_msgResponse& response){
  float* outShow;
