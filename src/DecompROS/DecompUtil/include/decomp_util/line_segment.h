  ...
  protected:
    ...
    template<int U = Dim>
      ...
      find_ellipsoid(double offset_x) {
        ...
        auto obs_inside = obs;
        int iii = 0;
        //**** decide short axes. Since the default value of  offset_x is 0.  The obs_inside.empty() may be true at the first time.
        while (!obs_inside.empty()) {
          ...
          obs_inside = obs_new;
          iii++;
          printf("times to find Ellipsoid %d\n", iii);
        }
    ...
    template<int U = Dim>
      ...
      while (!obs_inside.empty()) {
        ...
        vec_Vecf<Dim> obs_new;
        for(const auto &it: obs_inside) {
          if(1 - E.dist(it) > epsilon_)//如果障碍物在球内，则把障碍保留，用于确定另外一个半轴
            obs_new.push_back(it);
        }
        obs_inside = obs_new;
      }
